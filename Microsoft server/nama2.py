...
query = """
    SELECT
        ...
        sta.first_name as sales_first_name,
        sta.last_name as sales_last_name
    FROM 
        sales.orders ord
...
hasil = df.groupby(['order_id', 'cus_first_name', 'cus_last_name', 'city', 'state', 'order_date', 'product_name', 'category_name', 'brand_name', 'store_name', 'sales_first_name', 'sales_last_name'])[['quantity', 'revenue']].sum().reset_index()
print(hasil)

hasil['pelanggan'] = hasil['cus_first_name'] + ' ' + hasil['cus_last_name']
hasil['pelanggan2'] = hasil.apply(lambda x: " ".join([x.cus_first_name, x.cus_last_name]), axis=1)
hasil['pelanggan3'] = hasil.apply(lambda x: " ".join([x['cus_first_name'], x['cus_last_name']]), axis=1)
print(hasil)

hasil['sales'] = hasil['sales_first_name'] + ' ' + hasil['sales_last_name']
hasil['sales2'] = hasil.apply(lambda x: " ".join([x.sales_first_name, x.sales_last_name]), axis=1)
hasil['sales3'] = hasil.apply(lambda x: " ".join([x['sales_first_name'], x['sales_last_name']]), axis=1)
print(hasil)

hasil.to_csv('hasil otak-atik.csv', index=False)